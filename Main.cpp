                     ...
                     int       nCmdShow)
{
	...
	sscanf(lpCmdLine, "%s %d %d", imgFileName, &sizeOfVector, &sizeOfCodeBook);
	...
	//TO DO: validation of size of codebook being a power of 2

	// Set up the images 352x288
	int w = 352;
	int h = 288;
	inImage.setWidth(w);
	inImage.setHeight(h);
	inImage.setImagePath(imgFileName);
	inImage.ReadImageOneChannel();

	outImage = inImage;
	outImage.Modify(sizeOfCodeBook, sizeOfVector);
		...
		case WM_PAINT:
			{
				...
				SetDIBitsToDevice(hdc,
					inImage.getWidth() + 200, 100, outImage.getWidth(), outImage.getHeight(),
